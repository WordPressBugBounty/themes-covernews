  ...
  class AF_themes_info
  {
    ...
    public function covernews_register_info_page()
    {
      ...
      add_menu_page(
        ...
        'covernews', // Menu slug.
        array($this, 'covernews_render_page'), // Action.
        $afthemes_icon,
        30
      );
      ...
      add_submenu_page(
        'covernews', // Parent slug.
        __('Upgrade to Pro', 'covernews'), // Page title.
        '<span id="aft-upgrade-menu-item">' . __('Upgrade Now', 'covernews') . '</span>', // Menu title.
        'manage_options', // Capability.
    ...
    public function covernews_render_templates_kits()
    {
      ...
      if (($covernews_elespare_installed && $covernews_elespare_active) && $plugin_update == 'false' && is_plugin_active($covernews_elementor_installed)) {
        ...
        $class = (!empty($covernews_elespare_verison) && $covernews_elespare_active && $covernews_elespare_verison < $covernews_elespare_old_version)
          ...
          : 'false';
        echo '<div id="templatespare-plugin-install-activate" data-class="' . esc_attr($class) . '" current-theme="elespare" install="' . esc_attr(json_encode($install)) . '" activate="' . esc_attr(json_encode($activate)) . '" data-plugin-page="aft-template-kits" message="' . esc_attr($message) . '" isPro="' . esc_attr($covernews_elespare_status) . '"></div>';
      }
    }
